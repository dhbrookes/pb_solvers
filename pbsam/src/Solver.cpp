...
IE_orig_(ns, vector<double> (p*p*p*p)), calc_pts_(false),
  ...
  for (int n = 0; n < p_; n++)
  {
    for (int m = -n; m < n+1; m++)
    {
      for (int b = 0; b < Mp; b++)
      {
        ...
        hbj = make_hb_Ij(I, j, sph_grid[b], H, shcalc, besseli);
        hbj *= (4 * M_PI) / (Mp);
                         ...
                         shared_ptr<LHNMatrix> LHN)
{
  ...
  for (int k = 0; k < mol.get_ns(); k++)
  {
    ...
    for (int n = 0; n < p_; n++)
    {
      for (int m = - n; m < n+1; m++)
      {
        //TODO fix with altered LF and LH
